...
(() => {
  ...
  var cursorIconRevealIn = (scaleTo, icon) => {
    const animation = gsapWithCSS.timeline();
    animation.to(cursor, {
      padding: "1rem",
      width: scaleTo + "rem",
  ...
  var videoPlayer = () => {
    ...
    const muteButton = document.querySelector(".case-hero_mute-button");
    ...
    for (let i2 = 0; i2 < cursorAreas.length; i2++) {
      ...
      area.addEventListener("mouseenter", () => {
        if (iconType === void 0) {
          ...
          cursorIconRevealIn(scale2, areaIcon);
        }
      });
  ...
  var _getTarget = function _getTarget2(t2) {
    return gsap3.utils.toArray(t2)[0] || (typeof t2 === "string" && gsap3.config().nullTargetWarn !== false ? console.warn("Element not found:", t2) : null);
  };
  ...
  var _maxScroll = function _maxScroll2(element, _ref2) {
    ...
    return (s2 = "scroll" + d2) && (a = _getProxyProp(element, s2)) ? a() - _getBoundsFunc(element)()[d] : _isViewport3(element) ? (_docEl2[s2] || _body2[s2]) - (_win5["inner" + d2] || _docEl2["client" + d2] || _body2["client" + d2]) : element[s2] - element["offset" + d2];
  };
  ...
  var _getBounds = function _getBounds2(element, withoutTransforms) {
    var tween = withoutTransforms && _getComputedStyle3(element)[_transformProp2] !== "matrix(1, 0, 0, 1, 0, 0)" && gsap4.to(element, {
      x: 0,
      y: 0,
      xPercent: 0,
      yPercent: 0,
  ...
  var _getScrollNormalizer = function _getScrollNormalizer2(vars) {
    ...
    var _vars2 = vars, normalizeScrollX = _vars2.normalizeScrollX, momentum = _vars2.momentum, allowNestedScroll = _vars2.allowNestedScroll, self, maxY, target = _getTarget(vars.target) || _docEl2, smoother = gsap4.core.globals().ScrollSmoother, smootherInstance = smoother && smoother.get(), content = _fixIOSBug && (vars.content && _getTarget(vars.content) || smootherInstance && vars.content !== false && !smootherInstance.smooth() && smootherInstance.content()), scrollFuncY = _getScrollFunc(target, _vertical), scrollFuncX = _getScrollFunc(target, _horizontal), scale2 = 1, initialScale = (Observer.isTouch && _win5.visualViewport ? _win5.visualViewport.scale * _win5.visualViewport.width : _win5.outerWidth) / _win5.innerWidth, wheelRefresh = 0, resolveMomentumDuration = _isFunction3(momentum) ? function() {
    ...
    vars.onPress = function() {
      ...
      scale2 = _round3((_win5.visualViewport && _win5.visualViewport.scale || 1) / initialScale);
      tween.pause();
  ...
  var _createElement3 = function _createElement4(type, ns) {
    var e2 = _doc7.createElementNS ? _doc7.createElementNS((ns || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), type) : _doc7.createElement(type);
    return e2.style ? e2 : _doc7.createElement(type);
  };
  ...
  var _isRoot = function _isRoot2(e2) {
    return !!(!e2 || e2 === _docElement3 || e2.nodeType === 9 || e2 === _doc7.body || e2 === _win7 || !e2.nodeType || !e2.parentNode);
  };
  var _getMaxScroll = function _getMaxScroll2(element, axis) {
    var dim = axis === "x" ? "Width" : "Height", scroll = "scroll" + dim, client = "client" + dim;
    return Math.max(0, _isRoot(element) ? Math.max(_docElement3[scroll], _body4[scroll]) - (_win7["inner" + dim] || _docElement3[client] || _body4[client]) : element[scroll] - element[client]);
  };
  ...
  var _parseRect = function _parseRect2(e2) {
    if (e2 === _win7) {
      ...
      _tempRect.width = _tempRect.right = _docElement3.clientWidth || e2.innerWidth || _body4.clientWidth || 0;
      _tempRect.height = _tempRect.bottom = (e2.innerHeight || 0) - 20 < _docElement3.clientHeight ? _docElement3.clientHeight : e2.innerHeight || _body4.clientHeight || 0;
      return _tempRect;
    }
    var doc = e2.ownerDocument || _doc7, r = !_isUndefined3(e2.pageX) ? {
  ...
  var _isFixed3 = function _isFixed4(element) {
    if (_getComputedStyle5(element).position === "fixed") {
      return true;
    }
    element = element.parentNode;
    if (element && element.nodeType === 1) {
    ...
    function Draggable2(target, vars) {
      ...
      var type = (vars.type || "x,y").toLowerCase(), xyMode = ~type.indexOf("x") || ~type.indexOf("y"), rotationMode = type.indexOf("rotation") !== -1, xProp = rotationMode ? "rotation" : xyMode ? "x" : "left", yProp = xyMode ? "y" : "top", allowX = !!(~type.indexOf("x") || ~type.indexOf("left") || type === "scroll"), allowY = !!(~type.indexOf("y") || ~type.indexOf("top") || type === "scroll"), minimumMovement = vars.minimumMovement || 2, self = _assertThisInitialized2(_this2), triggers = _toArray3(vars.trigger || vars.handle || target), killProps = {}, dragEndTime = 0, checkAutoScrollBounds = false, autoScrollMarginTop = vars.autoScrollMarginTop || 40, autoScrollMarginRight = vars.autoScrollMarginRight || 40, autoScrollMarginBottom = vars.autoScrollMarginBottom || 40, autoScrollMarginLeft = vars.autoScrollMarginLeft || 40, isClickable = vars.clickableTest || _isClickable, clickTime = 0, gsCache = target._gsap || gsap5.core.getCache(target), isFixed = _isFixed3(target), getPropAsNum = function getPropAsNum2(property, unit) {
        ...
        if (xyMode) {
          ...
          self.y = parseFloat(target.style.top || (cs = _getComputedStyle5(target)) && cs.top) || 0;
        ...
        if (scrollProxy) {
          ...
          if (rotationMode) {
            parsedOrigin = target.ownerSVGElement ? [gsCache.xOrigin - target.getBBox().x, gsCache.yOrigin - target.getBBox().y] : (_getComputedStyle5(target)[_transformOriginProp3] || "0 0").split(" ");
        ...
        var originalEvent = e2, wasDragging = self.isDragging, isContextMenuRelease = self.vars.allowContextMenu && e2 && (e2.ctrlKey || e2.which > 2), placeholderDelayedCall = gsap5.delayedCall(1e-3, removePlaceholder), touches, i2, syntheticEvent, eventTarget, syntheticClick;
        if (touchEventTarget) {
  ...
  var _getGSAP7 = function _getGSAP8() {
    return gsap6 || typeof window !== "undefined" && (gsap6 = window.gsap);
  };
  var _lookup2 = {};
  ...
  var _initCore9 = function _initCore10() {
    ...
    if (gsap6) {
      ...
      _getCache3 = gsap6.core.getCache;
      _ticker2 = gsap6.ticker;
      _coreInitted6 = 1;
    }
  };
  ...
  var VelocityTracker = /* @__PURE__ */ function() {
    ...
    VelocityTracker2.track = function track(targets, properties, types) {
      ...
      var result = [], targs = _toArray4(targets), a = properties.split(","), t2 = (types || "").split(","), i2 = targs.length, tracker, j;
      while (i2--) {
  ...
  _getGSAP7() && gsap6.registerPlugin(VelocityTracker);
  ...
  var _initCore11 = function _initCore12() {
    gsap7 = _getGSAP9();
    if (gsap7) {
      _parseEase3 = gsap7.parseEase;
      _toArray5 = gsap7.utils.toArray;
      _getUnit2 = gsap7.utils.getUnit;
      _getCache4 = gsap7.core.getCache;
      ...
      PropTween2 = gsap7.core.PropTween;
      gsap7.config({
        resistance: 100,
        unitFactors: {
          time: 1e3,
      ...
      _config2 = gsap7.config();
      gsap7.registerPlugin(VelocityTracker);
      _coreInitted7 = 1;
    }
  };
  var InertiaPlugin2 = {
    ...
    init: function init4(target, vars, tween, index, targets) {
      ...
      var cache = target._gsap, getVal = cache.get, dur = vars.duration, durIsObj = _isObject7(dur), preventOvershoot = vars.preventOvershoot || durIsObj && dur.overshoot === 0, resistance = _getNumOrDefault(vars, "resistance", _config2.resistance), duration2 = _isNumber5(dur) ? dur : _calculateTweenDuration(target, vars, durIsObj && dur.max || 10, durIsObj && dur.min || 0.2, durIsObj && "overshoot" in dur ? +dur.overshoot : preventOvershoot ? 0 : 1, true), p, curProp, curVal, unit, velocity, change1, end, change2, linkedProps;
  ...
  _getGSAP9() && gsap7.registerPlugin(InertiaPlugin2);
  ...
  var careersPageReveal = () => {
    ...
    gsapWithCSS.set(heroReveal, { y: "0%" });
    const animation = gsapWithCSS.timeline({
      onComplete: () => {
        textSplitParent.revert();
        headerSplit.revert();
  ...
  var mediaGridReveal = () => {
    const overviewSection = document.querySelector(".section_careers-media");
  ...
  var cmsMediaGrid = () => {
    ...
    for (const i2 in mediaGrids) {
      const tempGrid = mediaGrids[i2];
      ...
      for (const j in fillContainers) {
        const container = fillContainers[j];
        const replaceImage = feedContent[j];
  ...
  gsapWithCSS.registerPlugin(SplitText);
  var csPageReveal = () => {
    const fixedHeaderSection = document.querySelector(".section_fixed-header");
  ...
  var heroZText = () => {
    const maskedTrack = document.querySelector(".home-hero_text-track");
    const bgTrack = document.querySelector(".bg-text_track");
    ...
    const midAnimation = gsapWithCSS.timeline({ repeat: -1, yoyo: true });
    midAnimation.fromTo(
      midChildren,
      { duration: textSpeed, x: textMovement + "%", ease: "linear" },
      { duration: textSpeed, x: -textMovement + "%", ease: "linear" }
    );
    const outAnimation = gsapWithCSS.timeline({ repeat: -1, yoyo: true });
    outAnimation.fromTo(
      outChildren,
      { duration: textSpeed, x: -textMovement + "%", ease: "linear" },
      { duration: textSpeed, x: textMovement + "%", ease: "linear" }
