...
(() => {
  ...
  var cursorMovement = () => {
    ...
    if (device12 === "tablet" || device12 === "mobile") {
      gsapWithCSS.set(cursor2, { display: "none" });
    }
    ...
    ScrollTrigger3.enable = function enable() {
      ...
      if (gsap6) {
        ...
        if (_body4) {
          ...
          var bodyStyle = _body4.style, border = bodyStyle.borderTopStyle, AnimationProto = gsap6.core.Animation.prototype, bounds, i2;
  ...
  var filterContentUpdated = () => {
    ...
    filterForm.addEventListener("submit", (e2) => {
      e2.preventDefault();
    });
    for (const i2 in filterCheckboxes) {
      const tempCheckbox = filterCheckboxes[i2];
      tempCheckbox.addEventListener("click", (e2) => {
        handleFilterClick(e2, masterList, activeFilters, renderQueue, initialFilter);
      });
    }
    searchInput.addEventListener("input", (e2) => {
      const searchTerm = e2.target.value.toLowerCase();
      const filteredList = filterList2(masterList, activeFilters);
      const searchResults = searchList(filteredList, searchTerm);
      renderUpdate(searchResults);
    });
    function handleFilterClick(e2, masterList2, activeFilters2, renderQueue2, initialFilter2) {
      const clickedCheckbox = e2.target;
      const clickedParent = clickedCheckbox.parentElement;
      const clickedSpan = clickedParent.querySelector("span");
      let filterText = clickedSpan.innerHTML;
      filterText = filterText.split(" ")[0];
      renderQueue2 = masterList2;
      if (clickedCheckbox.checked === true) {
        clickedSpan.style.color = "#EEEBE6";
        if (filterText !== "All") {
          activeFilters2.push(filterText);
          const tempList = filterList2(renderQueue2, activeFilters2);
          renderQueue2 = tempList;
          hideAll3();
          filterReveal(renderQueue2);
          updateInitialCheckbox2(initialFilter2, "hide");
        }
        if (filterText === "All") {
          activeFilters2 = [];
          renderQueue2 = masterList2;
        ...
        clickedSpan.style.color = "#EC2543";
        if (filterText !== "All") {
          const updatedFilters = activeFilters2.filter((item) => item !== filterText);
          activeFilters2 = updatedFilters;
          if (activeFilters2.length < 1) {
            hideAll3();
            filterReveal(renderQueue2);
            updateInitialCheckbox2(initialFilter2, "show");
            updateScrollEffect();
          } else {
            const tempList = filterList2(renderQueue2, activeFilters2);
            renderQueue2 = tempList;
    ...
    function updateInitialCheckbox2(initialFilter2, setState) {
      const checkboxContainer = initialFilter2.children[0];
      const checkboxInput = initialFilter2.children[1];
      const checkboxText = initialFilter2.children[2];
      if (setState === "hide") {
        checkboxInput.checked = false;
        checkboxContainer.classList.remove("w--redirected-checked");
        checkboxText.style.color = "#EC2543";
    ...
    function allReset2() {
      const filterCheckboxes2 = [...document.querySelectorAll("[data-filter-checkbox]")];
      for (let i2 = 0; i2 < filterCheckboxes2.length; i2++) {
        const temp = filterCheckboxes2[i2];
        const tempParent = temp.parentElement;
        const tempContainer = tempParent.children[0];
        const tempText = tempParent.querySelector("span");
        if (i2 !== 0) {
  ...
  var searchList = (items, searchTerm) => {
    if (!searchTerm)
      return items;
  ...
  var servicesScrollEffect = () => {
    ...
    function scrollEffect() {
      ...
      sMarkers.forEach((marker, i2) => {
        const isFirst = i2 === 0, isLast = i2 === sMarkers.length - 1;
        ...
        if (!isFirst) {
          ...
          const outHeader = splitMain[i2 - 1][0];
    ...
    function setupMarkers() {
      ...
      if (device10 === "mobile") {
        ...
        if (window.innerWidth > window.innerHeight) {
          ...
          servicesDescription.forEach((i2) => {
            gsapWithCSS.set(i2, { fontSize: "3.5vh" });
          });
      ...
      sMarkers.forEach((marker, i2) => {
        ...
        const st = gsapWithCSS.timeline({
          scrollTrigger: {
            ...
            onLeave: () => {
              if (!isLast) {
                gsapWithCSS.to(sSpans[i2].children[0], { y: st.progress() + 100 + "%" });
              }
            }
          }
        });
