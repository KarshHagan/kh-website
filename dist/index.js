...
(() => {
  ...
  var cursorRevealIn = (scaleTo, icon) => {
    if (icon !== void 0) {
  ...
  var cursorIconReset = () => {
    ...
    for (const i2 in cursorImages) {
      const temp = cursorImages[i2];
  ...
  var cursorHover = () => {
    ...
    for (let i2 = 0; i2 < cursorAreas.length; i2++) {
      ...
      area.addEventListener("mouseenter", () => {
        ...
        cursorRevealIn(scale2, areaIcon);
      });
  ...
  var menuOpenSplit = new SplitText("#menuText", { type: "words,chars" });
  var menuCloseSplit = new SplitText("#menuCloseText", { type: "words,chars" });
  var menuInit = (page) => {
  ...
  // src/motion/fixedHeader.ts
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  var fixedHeader = (section) => {
  ...
  // src/pages/about.ts
  var about = () => {
    const headerElement = document.querySelector("#fixedHeader");
  ...
  var caseOverview = () => {
    ...
    for (let i2 = 0; i2 < filterButtons.length; i2++) {
      let clicked = false;
      const temp = filterButtons[i2];
  ...
  var initSliderImages = () => {
    ...
    const checkSliderType = sliderImages[0];
  ...
  gsapWithCSS.registerPlugin(SplitText);
  var pItems = [...document.querySelectorAll(".home-featured_image-item")];
  var pTitles = [...document.querySelectorAll(".home-featured_title")];
  var pNumbers = [...document.querySelectorAll(".home-featured_number")];
  var pDescriptions = [...document.querySelectorAll(".home-featured_description")];
  var pButtons = [...document.querySelectorAll(".home-featured_button")];
  var initSliderContent = () => {
    for (let i2 = 0; i2 < pItems.length; i2++) {
      const titleTemp = pTitles[i2];
      const numberTemp = pNumbers[i2];
      const descTemp = pDescriptions[i2];
      const buttonTemp = pButtons[i2];
      numberTemp.innerHTML = "0" + (i2 + 1);
      if (i2 > 0) {
        gsapWithCSS.set([titleTemp, numberTemp], { y: "100%" });
        gsapWithCSS.set(descTemp, { opacity: 0 });
        gsapWithCSS.set(buttonTemp, { display: "none" });
      }
    }
  };
  var showNextContent = (curItem, prevItem) => {
    const moveCurTitle = pTitles[curItem];
    const movePrevTitle = pTitles[prevItem];
    const moveCurNumber = pNumbers[curItem];
    const movePrevNumber = pNumbers[prevItem];
    const moveCurDescription = pDescriptions[curItem];
    const movePrevDescription = pDescriptions[prevItem];
    const moveCurButton = pButtons[curItem];
    const movePrevButton = pButtons[prevItem];
    const infoAnimation = gsapWithCSS.timeline();
    infoAnimation.to(
  ...
  var showPrevContent = (curItem, prevItem) => {
    const moveCurTitle = pTitles[curItem];
    const movePrevTitle = pTitles[prevItem];
    const moveCurNumber = pNumbers[curItem];
    const movePrevNumber = pNumbers[prevItem];
  ...
  var scale = 0.05;
  var serviceImageHover = (movementX, movementY) => {
  ...
  var services2 = () => {
    ...
    servicesContainer.addEventListener("mousemove", (e2) => {
      serviceImageHover(e2.clientX, e2.clientY);
    });
  };
  ...
  var _getByTarget = function _getByTarget2(target) {
    return _lookup[_getID(typeof target === "string" ? _toArray3(target)[0] : target)];
  };
  ...
  _getGSAP5() && gsap5.registerPlugin(VelocityTracker);
  ...
  var _emptyFunc3 = function _emptyFunc4(p) {
    return p;
  };
  var _bigNum3 = 1e10;
  ...
  var _extend = function _extend2(obj, defaults2, exclude) {
    for (var p in defaults2) {
  ...
  var _deepClone = function _deepClone2(obj) {
    var copy = {}, p, v;
    for (p in obj) {
      copy[p] = _isObject5(v = obj[p]) && !_isArray2(v) ? _deepClone2(v) : v;
    }
    return copy;
  };
  var _getClosest = function _getClosest2(n2, values, max, min, radius) {
    var i2 = values.length, closest = 0, absDif = _bigNum3, val, dif, p, dist;
    if (_isObject5(n2)) {
      while (i2--) {
        val = values[i2];
        dif = 0;
  ...
  var _parseEnd = function _parseEnd2(curProp, end, max, min, name, radius, velocity) {
    ...
    if (_isObject5(end)) {
      ...
      adjustedEnd = _isFunction5(endVar) ? endVar(end, velocity) : _isArray2(endVar) ? _getClosest(end, endVar, max, min, radius) : parseFloat(endVar);
    }
    if (adjustedEnd > max) {
      adjustedEnd = max;
  ...
  var _calculateTweenDuration = function _calculateTweenDuration2(target, vars, maxDuration, minDuration, overshootTolerance, recordEnd) {
    ...
    var duration2 = 0, clippedDuration = _bigNum3, inertiaVars = vars.inertia || vars, getVal = _getCache4(target).get, resistance = _getNumOrDefault(inertiaVars, "resistance", _config2.resistance), p, curProp, curDuration, curVelocity, curVal, end, curClippedDuration, tracker, unitFactor, linkedProps;
  ...
  var InertiaPlugin = {
    ...
    init: function init4(target, vars, tween, index, targets) {
      ...
      var cache = target._gsap, getVal = cache.get, dur = vars.duration, durIsObj = _isObject5(dur), preventOvershoot = vars.preventOvershoot || durIsObj && dur.overshoot === 0, resistance = _getNumOrDefault(vars, "resistance", _config2.resistance), duration2 = _isNumber5(dur) ? dur : _calculateTweenDuration(target, vars, durIsObj && dur.max || 10, durIsObj && dur.min || 0.2, durIsObj && "overshoot" in dur ? +dur.overshoot : preventOvershoot ? 0 : 1, true), p, curProp, curVal, unit, velocity, change1, end, change2, linkedProps;
      ...
      for (p in vars) {
        if (!_reservedProps2[p]) {
          ...
          this._pt = new PropTween2(this._pt, target, p, curVal, 0, _emptyFunc3, 0, cache.set(target, p, this));
          this._pt.u = unit || 0;
          this._pt.c1 = change1;
          this._pt.c2 = change2;
        }
  ...
  _getGSAP7() && gsap6.registerPlugin(InertiaPlugin);

  // src/motion/homeMotion.ts
  gsapWithCSS.registerPlugin([SplitText, InertiaPlugin]);
  var heroScroll = () => {
  ...
  var featuredReveal = () => {
    const featuredSection = document.querySelector(".section_home-featured");
    const featuedContent = featuredSection.querySelector(".home-featured_info-item");
  ...
  var inertiaMovement = () => {
    ...
    for (const i2 in scrollSections) {
      ...
      tempSection.addEventListener("wheel", (e2) => {
